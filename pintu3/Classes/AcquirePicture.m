...
@interface AcquirePicture () <AVCaptureFileOutputRecordingDelegate>
...
// Preview Switcheroo
@property (nonatomic) Boolean previewShowing;
...
@implementation AcquirePicture

static NSString *const reuseIdentifierPhotoCell = @"photoCell";
static NSString *const reuseIdentifierPreviewCell = @"previewCell";
...
- (void)viewDidLoad
{
    ...
    dispatch_async( self.sessionQueue, ^{
        ...
        AVCaptureDevice *videoDevice = [AcquirePicture deviceWithMediaType:AVMediaTypeVideo preferringPosition:AVCaptureDevicePositionBack];
...
- (IBAction)toggleMovieRecording:(id)sender
{
    ...
    dispatch_async( self.sessionQueue, ^{
        if ( ! self.movieFileOutput.isRecording ) {
            ...
            [AcquirePicture setFlashMode:AVCaptureFlashModeOff forDevice:self.videoDeviceInput.device];
...
- (IBAction)changeCamera:(id)sender
{
    ...
    dispatch_async( self.sessionQueue, ^{
        ...
        AVCaptureDevice *videoDevice = [AcquirePicture deviceWithMediaType:AVMediaTypeVideo preferringPosition:preferredPosition];
        ...
        if ( [self.session canAddInput:videoDeviceInput] ) {
            ...
            [AcquirePicture setFlashMode:AVCaptureFlashModeAuto forDevice:videoDevice];
...
- (IBAction)snapStillImage:(id)sender
{
    dispatch_async( self.sessionQueue, ^{
        ...
        [AcquirePicture setFlashMode:AVCaptureFlashModeAuto forDevice:self.videoDeviceInput.device];
...
- (NSInteger)collectionView:(UICollectionView *)view numberOfItemsInSection:(NSInteger)section {
    ...
    return self.previewShowing ? self.assets.count : self.assets.count+1;
}

#define kImageViewTag 1 // the image view inside the collection view cell prototype is tagged with "1"
- (UICollectionViewCell *)collectionView:(UICollectionView *)cv cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    
    if (!self.previewShowing && indexPath.row == 0) {
        PreviewCell *cell = [cv dequeueReusableCellWithReuseIdentifier:reuseIdentifierPreviewCell forIndexPath:indexPath];
        NSLog(@"showing preview in collectionview");
        // TODO: moving the session over takes around TEN seconds
        dispatch_async(self.sessionQueue, ^(void){
            cell.previewView.session = self.session;
        });
        //[self.replicatorLayer addSublayer:cell.previewView.layer];
        return cell;
        ...
        ALAsset *asset = self.assets[self.previewShowing ? indexPath.row : indexPath.row-1];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (!self.previewShowing && indexPath.row == 0) {
        ...
        // TODO: moving the session over takes around TEN seconds
        dispatch_async(self.sessionQueue, ^(void){
            [self.busy setHidden:NO];
            [self.bigImageView setHidden:YES];
            self.previewView.session = self.session;
            [self.busy setHidden:YES];
        });
        [self.collectionView reloadData];
    } else {
